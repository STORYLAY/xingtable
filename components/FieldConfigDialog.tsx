...
export const DateTimePickerPopup: React.FC<{
  ...
  rowId?: string;
  colId?: string;
}> = ({ type, isDateTime, value, onSelect, onClose, anchorRef, rect, rowId, colId }) => {
  ...
  const init = useMemo(() => {
    ...
    if (value && value !== 'current_date' && value !== 'current_datetime' && value !== 'current_time') {
      try {
  ...
  useLayoutEffect(() => {
    const updatePosition = () => {
      if (popupRef.current) {
        ...
        if (activeRect) {
          const shouldShowAbove = activeRect.bottom + popupHeight > window.innerHeight;
          top = shouldShowAbove 
            ? Math.max(8, activeRect.top - popupHeight - 4) 
            : Math.min(window.innerHeight - popupHeight - 8, activeRect.bottom + 4);
        ...
        setCoords(prev => {
          if (prev.top === top && prev.left === left && prev.calculated) return prev;
  ...
  useEffect(() => {
    if (hourListRef.current) {
      const selectedEl = hourListRef.current.querySelector('[data-selected="true"]');
      if (selectedEl) {
        selectedEl.scrollIntoView({ block: 'center', behavior: 'auto' });
      }
    }
    if (minuteListRef.current) {
      const selectedEl = minuteListRef.current.querySelector('[data-selected="true"]');
      if (selectedEl) {
        selectedEl.scrollIntoView({ block: 'center', behavior: 'auto' });
      }
    }
        ...
        {type !== FieldType.TIME && (
          <div className="p-4 w-[280px] relative">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-1 font-medium text-gray-800">
                ...
                >
                  ...
                  <ICONS.ChevronDown className={`w-3.5 h-3.5 text-gray-500 transition-transform duration-200 ${showYearSelect ? 'rotate-180' : ''}`} />
                </button>
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  ...
                  <ICONS.ChevronDown className={`w-3.5 h-3.5 text-gray-500 transition-transform duration-200 ${showMonthSelect ? 'rotate-180' : ''}`} />
                </button>
              </div>
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`w-7 h-7 flex items-center justify-center hover:bg-gray-100 rounded-full text-gray-500 transition-opacity ${
                    (showYearSelect || showMonthSelect) ? 'opacity-20 cursor-not-allowed' : 'cursor-pointer'
                  }`}
                >
            ...
            {showYearSelect && (
              <div className="absolute top-12 left-0 right-0 bottom-0 bg-white z-10 px-4 py-2 flex flex-col">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">选择年份</span>
                  <button
                    type="button"
                ...
                >
                  {Array.from({ length: 101 }, (_, i) => now.getFullYear() - 50 + i).map((y) => (
                    <button
                      type="button"
                      key={y}
            ...
            {showMonthSelect && (
              <div className="absolute top-12 left-0 right-0 bottom-0 bg-white z-10 px-4 py-2 flex flex-col">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">选择月份</span>
                  <button
                    type="button"
          ...
          <section>
            ...
            <Select 
              ...
              onChange={(val) => {
                ...
                if (defaultValue && defaultValue !== 'current_date' && defaultValue !== 'current_datetime') {
  ...
  const renderDefaultValueInput = () => {
    ...
    switch (type) {
      ...
      case FieldType.DATE: {
        ...
        const isCurrentSelected = defaultValue === 'current_date' || defaultValue === 'current_datetime';
        ...
        return (
          <div ref={dateContainerRef} className="relative w-full">
            ...
            >
              <span className={`truncate text-xs ${!defaultValue ? 'text-gray-400' : 'text-gray-700'}`}>
                {getTriggerLabel()}
              </span>
              <ICONS.ChevronDown className={`w-4 h-4 shrink-0 text-gray-400 transition-transform duration-200 ${isDateSelectOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            {isDateSelectOpen && (
              <div className="absolute left-0 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl z-50 py-1 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                >
                  <span>不选择默认值</span>
                  {!defaultValue && <ICONS.Check className="w-4 h-4 text-primary-600 shrink-0 ml-2" />}
                </div>
                
                <div
                  onClick={() => {
                    setDefaultValue(isDateTime ? 'current_datetime' : 'current_date');
                    setIsDateSelectOpen(false);
                    setIsDateCalendarOpen(false);
                  }}
                ...
                >
                  <span>{isDateTime ? '添加新记录的创建时间' : '添加新记录的创建日期'}</span>
                  {isCurrentSelected && <ICONS.Check className="w-4 h-4 text-primary-600 shrink-0 ml-2" />}
                </div>
                ...
                >
                  <span>{isDateTime ? '选择具体日期和时间' : '选择具体日期'}</span>
                  <svg className="w-3.5 h-3.5 text-gray-400 shrink-0 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </div>
              </div>
        ...
        return (
          <div ref={timeContainerRef} className="relative w-full">
            ...
            >
              ...
              <ICONS.ChevronDown className={`w-4 h-4 shrink-0 text-gray-400 transition-transform duration-200 ${isTimeSelectOpen ? 'rotate-180' : ''}`} />
            </button>
